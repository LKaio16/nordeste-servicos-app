...
class _OrcamentoEditScreenState extends ConsumerState<OrcamentoEditScreen> {
  ...
  void initState() {
    super.initState();
    _dataValidade = DateTime.now(); // Valor inicial padrão
    // Inicia o carregamento dos dados do orçamento
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref.read(orcamentoEditProvider(widget.orcamentoId).notifier).loadInitialData(widget.orcamentoId);
    });
  }
  ...
  void _submitForm() async {
    if (_formKey.currentState!.validate()) {
      ...
      final orcamentoAtualizado = Orcamento(
        ...
        status: state.originalOrcamento!.status,
      ...
      if (success && mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Orçamento atualizado!'), backgroundColor: AppColors.successGreen));
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    // Listener para preencher o formulário quando os dados chegam
    ref.listen<OrcamentoEditState>(orcamentoEditProvider(widget.orcamentoId), (previous, next) {
      if (next.originalOrcamento != null && !_initialDataLoaded) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Editar Orçamento', style: GoogleFonts.poppins(fontWeight: FontWeight.w600, color: Colors.white)),
        ...
        actions: [
          Padding(
            ...
            child: TextButton.icon(
              ...
              icon: state.isSubmitting ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2)) : const Icon(Icons.check_circle_outline, color: Colors.white),
