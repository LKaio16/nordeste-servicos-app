...
import '../../providers/auth_provider.dart'; // Importe o provider de autenticação
...
class LoginScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // TODO: Adicionar um Listener para navegar após o login bem-sucedido
    /*
    ref.listen<AuthState>(authProvider, (previousState, newState) {
      if (newState.isAuthenticated && previousState?.isAuthenticated == false) {
        // Navegar para a tela principal (ex: Dashboard)
         Navigator.of(context).pushReplacementNamed('/dashboard'); // Exemplo de navegação
      }
    ...
    */
    ...
    return Scaffold(
      ...
      body: Center( // Centraliza o conteúdo
        child: SingleChildScrollView( // Permite scrollar se o conteúdo for maior que a tela
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              // Botão de Login
              ElevatedButton(
                onPressed: authState.isLoading // Desabilita o botão enquanto estiver carregando
                    ? null
                    : () {
                ...
                style: ElevatedButton.styleFrom(
                  padding: const EdgeInsets.symmetric(vertical: 16.0), // Padding no botão
                  shape: RoundedRectangleBorder( // Borda arredondada
                    borderRadius: BorderRadius.circular(8.0),
                  ),
                ),
                child: authState.isLoading // Mostra um indicador de loading ou o texto do botão
                    ? const SizedBox(
                  height: 20.0,
                  width: 20.0,
                  child: CircularProgressIndicator(
                    valueColor: AlwaysStoppedAnimation<Color>(Colors.white), // Cor branca para o loading
                    strokeWidth: 3.0,
                  ),
                )
                    : const Text(
              ...
              const SizedBox(height: 12.0), // Espaço para a mensagem de erro

              // Exibe a mensagem de erro, se houver
              if (authState.errorMessage != null)
                Text(
                  authState.errorMessage!, // Usa o '!' pois verificamos que não é nulo
